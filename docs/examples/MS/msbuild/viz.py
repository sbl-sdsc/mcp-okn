...
FIG = f"{OUT}/figures"
BLUE="#2E86AB"; GREEN="#3B8C4D"; ORANGE="#F18F01"; PURPLE="#A23B72"; RED="#C0392B"
INK="#1c2733"; GRAY="#95a5a6"; MUTED="#6b7783"
plt.rcParams.update({"font.family":"DejaVu Sans","axes.edgecolor":"#c9d3dc","axes.linewidth":0.8})
...
xs = [1,2,3]; ys = [nsrc.get(str(i),0) for i in xs]
cols = [GRAY, BLUE, RED]
bars = ax1.bar([str(x) for x in xs], ys, color=cols, edgecolor="#333", width=0.62)
for b,y in zip(bars,ys):
    ax1.text(b.get_x()+b.get_width()/2, y+3, str(y), ha="center", fontsize=12, fontweight="bold")
ax1.set_title("MS-associated genes by number of corroborating sources", fontsize=13, fontweight="bold")
ax1.set_xlabel("# independent association sources (spoke-okn / rdkg / digcfdekg)", fontsize=10.5)
ax1.set_ylabel("number of genes", fontsize=10.5)
ax1.set_ylim(0, max(ys)*1.16)
t1 = stats["tier1_genes_3sources"]
box = ("13 genes in all 3 sources (T1 high-confidence core):\n"
       "HLA-DRB1, IL2RA, IL7R, TYK2, STAT4, CD6, CD40,\nCD58, CBLB, IL12A, IFNG, TNFRSF1A, TNFSF14")
ax1.text(0.97, 0.93, box, transform=ax1.transAxes, ha="right", va="top", fontsize=9.2, style="italic",
         bbox=dict(boxstyle="round,pad=0.5", fc="#FFF6E6", ec=ORANGE, lw=1.2))
...
barh = ax2.barh(labels, vals, color=[cmap[l] for l in labels], edgecolor="#333", height=0.6)
for b,v in zip(barh,vals):
    ax2.text(v+3, b.get_y()+b.get_height()/2, str(v), va="center", fontsize=11.5, fontweight="bold")
...
ax2.text(0.97, 0.12, "spoke-okn: curated links\nrdkg: curated (immune/rare)\ndigcfdekg: statistical (GWAS/PIGEAN)",
         transform=ax2.transAxes, ha="right", va="bottom", fontsize=8.8, style="italic", color=MUTED)
...
ev_order = ["curated_link","statistical_association","measured_activity_change","pathway_membership"]
ev_lab = ["curated link","statistical /\ngenetic assoc.","measured\nactivity change","pathway\nmembership"]
...
for b,v in zip(bars,ev_val):
    ax1.text(b.get_x()+b.get_width()/2, v+3, str(v), ha="center", fontsize=12, fontweight="bold")
ax1.set_title("Findings by evidence type (kept separate)", fontsize=13, fontweight="bold")
...
ent_map = [("gene (association)","gene",BLUE),("gene: altered activity","gene_altered_activity",ORANGE),
           ("drug / therapeutic","drug",PURPLE),("pathway / gene set","pathway_or_geneset",GREEN),
           ("clinical feature","clinical_feature",RED),("biomarker","biomarker","#16a085"),
           ("genetic variant","genetic_variant",GRAY)]
elabels=[m[0] for m in ent_map]; evals=[ent.get(m[1],0) for m in ent_map]; ecols=[m[2] for m in ent_map]
order = np.argsort(evals)
elabels=[elabels[i] for i in order]; evals=[evals[i] for i in order]; ecols=[ecols[i] for i in order]
barh = ax2.barh(elabels, evals, color=ecols, edgecolor="#333", height=0.66)
for b,v in zip(barh,evals):
    ax2.text(v+3, b.get_y()+b.get_height()/2, str(v), va="center", fontsize=11, fontweight="bold")
...
for i,r in enumerate(top):
    for j,s in enumerate(srcs):
        ...
        ax.add_patch(plt.Rectangle((j,i),0.94,0.94, facecolor=color, edgecolor="white", lw=1.4))
ax.set_xlim(0,4); ax.set_ylim(0,len(top))
ax.set_xticks([j+0.47 for j in range(4)])
ax.set_xticklabels(["spoke-okn\n(curated)","rdkg\n(curated)","digcfdekg\n(statistical)","GXA\n(measured DE)"], fontsize=9)
ax.set_yticks([i+0.47 for i in range(len(top))])
lab = []
for r in top:
    w = r["digcfdekg_weight"]; t = r["confidence_tier"].split()[0]
