...
def gcol(g):
    """Return the plot color for a gene based on its confidence tier."""
    return {"T1": RED, "T2": ORANGE, "T3": BLUE}.get(tier.get(g, "T4"), BLUE)
...
R = 6.6
mod_pos = {}
for name, ang, _genes, _drugs in MODULES:
    a = math.radians(ang)
    mod_pos[name] = (R * math.cos(a), R * math.sin(a))
    ax.plot(
        [0, mod_pos[name][0]], [0, mod_pos[name][1]], color="#cfd8e0", lw=1.5, zorder=1
    )

ax.scatter(
    [0], [0], marker="*", s=2800, color=RED, edgecolor="#7b241c", lw=1.5, zorder=6
)
ax.text(
    0,
    -1.05,
...
for name, ang, genes, drugs in MODULES:
    ...
    for i, g in enumerate(genes):
        ...
        ga = a + off
        gx = mx + ring * math.cos(ga)
        gy = my + ring * math.sin(ga)
        ax.plot(
            [mx + 0.9 * math.cos(ga), gx],
            [my + 0.9 * math.sin(ga), gy],
            color="#dce4eb",
            lw=0.8,
            zorder=2,
        )
    ...
    for k, (dname, dnote) in enumerate(drugs):
        ...
        dx = (R + 5.0) * math.cos(da)
        dy = (R + 5.0) * math.sin(da)
        ax.add_patch(
            RegularPolygon(
                (dx, dy),
                3,
        ...
        ax.text(
            ...
            dy,
            f"{dname}\n({dnote})",
            ha=("left" if math.cos(da) >= 0 else "right"),
            va="center",
            fontsize=8,
            color="#7a4a06",
            zorder=5,
...
leg = [
    ...
    Line2D(
        ...
        markersize=13,
        label="Mechanistic module / pathway",
    ),
    Line2D(
        [0],
        [0],
...
ax.axis("off")
ax.set_title(
    "MS gene–pathway–drug mechanistic map  (entities retrieved from Proto-OKN sources)",
    fontsize=15,
    fontweight="bold",
    pad=10,
)
plt.tight_layout()
plt.savefig(
    f"{OUT}/figures/fig3_gene_pathway_drug_network.png", dpi=140, bbox_inches="tight"
)
plt.close()
print("fig3 done")
