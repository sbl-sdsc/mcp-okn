...
<div class="note" style="margin-top:10px"><b>Why "excessive drinking" shows a spurious −0.63 — and why it is <i>not</i> protective.</b> The raw values are intact (age-adjusted % of adults reporting binge/heavy drinking, mean 19.1% per county, stored as <code>value(SE)</code> and parsed correctly), so the negative sign is not a data error. It is <b>ecological confounding by socioeconomic status</b>: at the county level excessive drinking behaves as an <i>affluence marker</i>. It correlates <b>positively</b> with high-school completion (+0.48), life expectancy (+0.48), some college (+0.40) and broadband access (+0.33), and <b>negatively</b> with children in poverty (−0.51), food insecurity (−0.54), physical inactivity (−0.58), obesity (−0.42) and smoking (−0.37). Wealthier, better-educated counties (Upper Midwest, Mountain West, urban) report <i>more</i> binge/heavy drinking but have <i>less</i> diabetes, whereas the poor rural "diabetes belt" reports <i>less</i> excessive drinking (more abstention and "dry" counties) but far <i>more</i> diabetes. The −0.63 therefore tracks the socioeconomic gradient rather than any effect of alcohol — a textbook ecological fallacy. It is down-tiered to <b>T4</b> in the findings CSV and greyed in the figure above; it should not be read as alcohol protecting against diabetes.</div>
