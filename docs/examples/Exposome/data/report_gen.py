...
def load(fn):
    ...
    p = f"{BASE}/data/{fn}"
    p = p if Path(p).exists() else f"{BASE}/{fn}"
    with Path(p).open() as f:
        return list(csv.DictReader(f))


def b64(fn):
    """Read a PNG figure and return it as a base64 data URI."""
    with Path(f"{BASE}/figures/{fn}").open("rb") as f:
...
gxa = load("gxa_expression.csv")
rdkg = load("rdkg_rare_disease.csv")
prokn = load("prokn_protein.csv")
spoke = load("spoke_gene_disease.csv")
rank = load("corroboration_ranking.csv")
det = load("corroboration_detail.csv")
oard = load("oard_disease_phenotype.csv")
fuse_by = defaultdict(list)
for r in fuse:
    fuse_by[r["cas"]].append(r["category"])
cas2ab = {r["cas"]: r["abbrev"] for r in chem}

with Path(f"{BASE}/findings_master.csv").open() as f:
    nfind = sum(1 for _ in f) - 1
ndis = len({r["disease"] for r in spoke})
bpa7 = sorted(
    {
        r["disease"]
        for r in rank
        if r["chemical"] == "BPA" and r["corroboration_score"] == "7"
    }
)
...
KGV = {
    r["shortname"]: r
    for r in [
        {"shortname": "biobricks-aopwiki", "version": "v0.0.4"},
        {"shortname": "biobricks-toxcast", "version": "v0.0.2"},
        {"shortname": "biobricks-tox21", "version": "v0.0.3"},
        {"shortname": "biobricks-ice", "version": "v0.0.3"},
        {"shortname": "biobricks-pubchem-annotations", "version": "v0.0.2"},
        {"shortname": "gene-expression-atlas-okn", "version": "v0.0.3"},
        {"shortname": "spoke-okn", "version": "v0.0.6"},
        {"shortname": "rdkg", "version": "v0.0.1"},
        {"shortname": "prokn", "version": "v0.0.5"},
        {"shortname": "oard-kg", "version": "v0.0.3"},
        {"shortname": "ubergraph", "version": "v0.0.2"},
        {"shortname": "sudokn", "version": "v0.0.10"},
        {"shortname": "sawgraph", "version": "v0.0.15"},
        {"shortname": "fiokg", "version": "v0.0.11"},
    ]
}
...
tbl_rows = [
    ("biobricks-aopwiki", "curated AOPs: MIE→KE→AO & chemical stressors"),
...
for r in chem:
    ...
    M.append(
        f"| **{r['abbrev']}** | {r['name']} | {r['cas']} | {r['pubchem_cid'] or '—'} | {r['dtxsid']} | "
...
for r in aop:
    M.append(
        f"| {r['chemical']} | AOP {r['aop_id']} | {r['mie_title']} | {r['adverse_outcome']} | {r['n_key_events']} | {r['curated_target']} |\n"
    )
M.append(
...
for r in targets:
    M.append(
        f"| **{r['symbol']}** | {r['name']} | {r['ensembl'] or '—'} | {r['entrez'] or '—'} | {r['uniprot']} | {r['role_in_bisphenol_MoA']} |\n"
    )
...
M.append(
    f"SPOKE-OKN supplies **{len(spoke)} gene→disease associations** across {ndis} DOID diseases; the estrogen/androgen receptors and PPARG dominate the hormone-sensitive-cancer, reproductive and cardiometabolic clusters. RDKG adds curated **rare-disease** associations (ESR1 124, PPARG 104, ESR2 73, AR 59 MONDO diseases). ProKN confirms every target protein with GO/Reactome annotations (ESR1: 15 Reactome pathways, 31 GO terms). OARD contributes **EHR disease→phenotype** profiles for the rare-disease subset — most relevantly **precocious puberty (1,639 phenotypes)** and **prostate cancer (1,218)**, both classic endocrine-disruption outcomes.\n"
)
...
for sn, url in [
    ...
    (
        ...
        "https://github.com/biobricks-ai/pubchem-annotations-kg",
    ),
...
with Path(f"{BASE}/bisphenol_exposome_report.md").open("w") as f:
    f.write(
        "".join(M).replace("/sessions/nifty-festive-gauss/mnt/outputs/exposome/", "")
    )
with Path(f"{BASE}/bisphenol_exposome_report.md").open() as f:
...
def chk(v):
    """Render a checkmark or dot cell for a boolean-ish flag value."""
    return '<span class="y">✓</span>' if v == "1" else '<span class="n">·</span>'
...
H = [
    ...
    "<title>The Chemical Exposome of Bisphenols — Proto-OKN Evidence Map</title><style>",
    CSS,
    "</style></head><body>",
]
H.append(
...
H.append(
    '<div class="callout warn"><b>Read with care.</b> AOP coverage is curated & sparse; assay activity is <i>in-vitro</i>, not <i>in-vivo</i> effect; several joins are ontology-bridged; and AOP-Wiki\'s automated key-event→gene links are unreliable (see §11), so targets were taken from the curated MIE biology.</div>'
)

H.append(
    "<h2>1 · Data provenance &amp; method</h2><table><tr><th>Knowledge graph</th><th>Ver.</th><th>Role in the map</th></tr>"
)
for sn, role in tbl_rows:
    H.append(
        f"<tr><td><code>{sn}</code></td><td>{KGV.get(sn, {}).get('version', '—')}</td><td>{role}</td></tr>"
    )
H.append("</table>")
H.append(
    "<p><b>Joins.</b> Chemicals on <b>CAS</b> (http/https CAS-IRI forms reconciled); genes on <b>Ensembl</b> &amp; <b>Entrez</b>; proteins on <b>UniProt</b>; diseases bridged <b>DOID↔MONDO</b> via Ubergraph (45/45 SPOKE diseases mapped). Each finding keeps its source, relationship type and evidence kind separate.</p>"
)

H.append("<h2>2 · Chemicals — identity, cross-walk &amp; functional use</h2>")
H.append(
    f'<figure><img src="{b64("fig1_assay_coverage.png")}"><figcaption>Figure 1 · Per-bisphenol ToxCast assay coverage and activity (binary hitcalls).</figcaption></figure>'
)
H.append(
    "<table><tr><th>Abbr</th><th>Name</th><th>CAS</th><th>CID</th><th>DTXSID</th><th>AOP</th><th>ToxCast</th><th>Tox21</th><th>ICE</th><th>ICE functional use</th></tr>"
)
for r in chem:
    fu = ", ".join(sorted(set(fuse_by.get(r["cas"], [])))) or "—"
    H.append(
        f"<tr><td><b>{r['abbrev']}</b></td><td>{esc(r['name'])}</td><td>{r['cas']}</td><td>{r['pubchem_cid'] or '—'}</td>"
...
H.append(
    "</table><p>Functional use separates the family: <b>BPA</b> = binder/catalyst/hardener (polycarbonate &amp; epoxy monomer); halogenated <b>TBBPA/TCBPA</b> = flame retardant; newer analogues = antioxidant/UV-absorber/colorant substitutes.</p>"
)

H.append(
    "<h2>3 · Adverse outcome pathways (AOP-Wiki)</h2><table><tr><th>Chemical</th><th>AOP</th><th>Molecular initiating event</th><th>Adverse outcome</th><th>KEs</th><th>Target</th></tr>"
)
for r in aop:
    H.append(
        f"<tr><td>{r['chemical']}</td><td>AOP {r['aop_id']}</td><td>{esc(r['mie_title'])}</td><td>{esc(r['adverse_outcome'])}</td><td>{r['n_key_events']}</td><td>{r['curated_target']}</td></tr>"
    )
H.append("</table>")

H.append(
    "<h2>4 · Molecular targets</h2><table><tr><th>Gene</th><th>Protein</th><th>Ensembl</th><th>Entrez</th><th>UniProt</th><th>Role</th></tr>"
)
for r in targets:
    H.append(
        f"<tr><td><b>{r['symbol']}</b></td><td>{esc(r['name'])}</td><td>{r['ensembl'] or '—'}</td><td>{r['entrez'] or '—'}</td><td>{r['uniprot']}</td><td>{esc(r['role_in_bisphenol_MoA'])}</td></tr>"
    )
H.append("</table>")

H.append("<h2>5 · Differential expression (Gene Expression Atlas)</h2>")
H.append(
    f'<figure><img src="{b64("fig2_expression_matrix.png")}"><figcaption>Figure 2 · Significant differential-expression contrasts (adj p&lt;0.05) per target; red=up, teal=down.</figcaption></figure>'
)
H.append(
    "<p>All 16 targets are differentially expressed across many disease contrasts — PPARG (399), GATA3 (374), AHR (290), GPER1 (284) lead by breadth; TTR shows the largest single effect (|log2FC|≤17.1).</p>"
)

H.append("<h2>6 · Targets → disease</h2>")
H.append(
    f'<figure><img src="{b64("fig3_target_disease_matrix.png")}"><figcaption>Figure 3 · Target gene → disease associations (SPOKE-OKN, DOID; top 20 diseases).</figcaption></figure>'
)
H.append(
    f"<p>SPOKE-OKN supplies {len(spoke)} gene→disease associations across {ndis} diseases; RDKG adds rare-disease genetics (ESR1 124, PPARG 104, ESR2 73, AR 59 MONDO diseases); ProKN annotates every target protein (ESR1: 15 Reactome, 31 GO); OARD gives EHR phenotypes for <b>precocious puberty (1,639)</b> and <b>prostate cancer (1,218)</b> — both endocrine-disruption outcomes.</p>"
)

H.append("<h2>7 · Integrated exposome flow</h2>")
H.append(
    f'<figure><img src="{b64("fig5_sankey.png")}"><figcaption>Figure 4 · Chemical → molecular target → disease category. Interactive version: <code>figures/sankey_chemical_target_disease.html</code>.</figcaption></figure>'
)

H.append("<h2>8 · Cross-source corroboration ranking</h2>")
H.append(
    f'<figure><img src="{b64("fig4_corroboration_bars.png")}"><figcaption>Figure 5 · Chemical→disease links scored by number of independent agreeing sources (max 7).</figcaption></figure>'
)
H.append(
    f'<p>Of {len(rank)} chemical→disease links: <span class="tag s7">{sc.get(7, 0)} score 7 (BPA)</span> <span class="tag s6">{sc.get(6, 0)} score 6 (TBBPA)</span> <span class="tag s5">{sc.get(5, 0)} score 5 (analogues)</span>. <b>Breast cancer</b> is the best-corroborated outcome — the only disease reached through three converging BPA targets (ERα, ERβ, GATA3) with all seven evidence types agreeing.</p>'
)
H.append(
    "<h3>Best-supported links</h3><table><tr><th>Chemical</th><th>Disease</th><th>Score</th><th>Mediating targets</th><th>Agreeing sources</th></tr>"
)
show = [
    r
    for r in rank
    if r["chemical"] in ("BPA", "TBBPA") and int(r["corroboration_score"]) >= 6
]
show = sorted(
    show, key=lambda r: (-int(r["corroboration_score"]), r["chemical"], r["disease"])
)[:16]
for r in show:
    ...
    srcs = (
        ...
        .replace("rare disease", "rare-disease")
        .replace("protein annot", "protein")
    )
    H.append(
        f'<tr><td>{r["chemical"]}</td><td>{esc(r["disease"])}</td><td><span class="tag {cls}">{r["corroboration_score"]}/7</span></td><td>{r["mediating_targets"].replace("|", ", ")}</td><td style="font-size:12px">{srcs.replace("|", " · ")}</td></tr>'
    )
H.append(
    '</table><p style="font-size:13px;color:var(--mut)">Analogues (BPS, BPF, BPAF, BPB, BPAP, BPE, BPC, BPZ, TCBPA) share the ERα/ERβ→disease evidence at score 5 and are further ranked by assay potency (BPAF &gt; BPB &gt; TCBPA &gt; … &gt; BPS &gt; BPF).</p>'
)

H.append(
    "<h2>9 · Industrial &amp; exposure context</h2>"
...
H.append(
    '<h2>10 · Uncertainties &amp; limitations</h2><div class="callout warn"><ol>'
...
print(
    "HTML report written:",
    Path(f"{BASE}/bisphenol_exposome_report.html").stat().st_size // 1024,
    "KB",
)
