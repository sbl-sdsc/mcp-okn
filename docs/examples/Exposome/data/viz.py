...
def load(fn):
    import os
    p=f"{D}/data/{fn}"
    if not os.path.exists(p): p=f"{D}/{fn}"
    with open(p) as f: return list(csv.DictReader(f))
plt.rcParams.update({"font.size":9,"axes.titlesize":12,"axes.titleweight":"bold","figure.dpi":150})
TEAL="#1b7a7a"; CORAL="#d1495b"; NAVY="#20365b"; AMBER="#e0a458"; GREY="#8a8f98"
...
pct=[100*a/t for a,t in zip(active,tested)]
...
for i,(t,a,p) in enumerate(zip(tested,active,pct)):
    ax.text(t+12,i,f"{a}/{t} ({p:.0f}%)",va="center",fontsize=8,color=NAVY)
ax.set_yticks(y); ax.set_yticklabels(labels); ax.invert_yaxis()
ax.set_xlabel("ToxCast high-throughput assay endpoints"); ax.set_xlim(0,1650)
...
plt.tight_layout(); plt.savefig(f"{FIG}/fig1_assay_coverage.png",bbox_inches="tight"); plt.close()

# ---- FIG 2: expression matrix (GXA targets) ----
gx=load("gxa_expression.csv")
gx.sort(key=lambda r:int(r["n_sig_contrasts"]),reverse=True)
sym=[r["symbol"] for r in gx]; up=[int(r["n_up"]) for r in gx]; dn=[int(r["n_down"]) for r in gx]
...
ax.set_yticks(y); ax.set_yticklabels(sym); ax.invert_yaxis()
ax.set_xlabel("← down-regulated    significant GXA contrasts (adj p<0.05)    up-regulated →")
ax.set_title("Differential expression of bisphenol molecular targets (Gene Expression Atlas)")
ax.legend(loc="lower right",frameon=False); ax.spines[["top","right"]].set_visible(False)
plt.tight_layout(); plt.savefig(f"{FIG}/fig2_expression_matrix.png",bbox_inches="tight"); plt.close()
...
ax.set_xticks(range(len(top_dis))); ax.set_xticklabels(top_dis,rotation=55,ha="right",fontsize=8)
ax.set_yticks(range(len(genes))); ax.set_yticklabels(genes,fontsize=8)
for i in range(len(genes)):
    for j in range(len(top_dis)):
        if M[i,j]: ax.text(j,i,"●",ha="center",va="center",color=NAVY,fontsize=7)
ax.set_title("Bisphenol target gene → disease associations (SPOKE-OKN, DOID)")
plt.tight_layout(); plt.savefig(f"{FIG}/fig3_target_disease_matrix.png",bbox_inches="tight"); plt.close()
...
for i,r in enumerate(sel):
    ax.text(sc[i]+0.05,i,r["sources"].replace("_"," ").replace("aop structure","AOP").replace("assay activity","assay").replace("disease assoc","disease").replace("rare disease","rare").replace("protein annot","protein"),va="center",fontsize=6,color="#555")
...
ax.legend(handles=[Patch(color=NAVY,label="AOP-anchored (curated pathway)"),Patch(color=TEAL,label="Assay/target-based")],loc="lower right",frameon=False)
ax.spines[["top","right"]].set_visible(False)
plt.tight_layout(); plt.savefig(f"{FIG}/fig4_corroboration_bars.png",bbox_inches="tight"); plt.close()
print("Static figures written:", *[f for f in __import__("os").listdir(FIG)])
