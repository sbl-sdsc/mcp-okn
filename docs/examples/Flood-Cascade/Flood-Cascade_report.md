...
> UF-OKN flood footprint (national, 47,512 buildings) intersected with the OKN
> federation's hydrologic network (507,486 NHDPlus reaches across 348 HUC8
> subbasins in 10 HUC2 regions). The level of inference is **spatial-topological
...
Starting from the UF-OKN modelled flood footprint (47,512 flood-impacted buildings
resolving to 2,738 S2 Level-13 cells nationally), we found **11,085 EPA-regulated
facilities** sitting inside 1,339 of those cells — a co-location rate of about 54% of
the flood cells that could be placed on the national grid. Flood exposure of regulated industry is
**moderately concentrated, not diffuse**: of the 2,270 facilities carrying a NAICS code,
manufacturing (NAICS 31–33) alone accounts for 799 (35.2%), the top
three sectors take 45.1%, and the sector Herfindahl–Hirschman Index is 908.0.

We then routed the water. Using `hydrologykg`'s precomputed NHDPlus transitive downstream closure,
1,101 stream reaches crossing a facility-bearing flood cell reach
15,727 distinct downstream reaches over 996,529 routed links, touching
25,063 downstream S2 cells. Rolled up to counties, this produces a **typology** rather
than a ranking: **153 Retained** counties (their contamination stays home), **149
Imported** counties (their exposure is generated upstream), **59 Compound** counties (heavy
on both), and 158 Low. **179 counties, home to
18.5 million people, have no flood-exposed regulated facility of their own yet sit
...
Imported counties are markedly more rural and smaller than Retained ones (62% vs
33% rural; median RUCC 6 vs 3; median
population 20,813 vs 107,215). Dropping the routing step and ranking on
co-location alone changes **31 of the top 50 counties** (Spearman ρ = 0.578,
Kendall τ = 0.475). And 151 of the 149+59
Imported/Compound counties — 8.4 million residents — have **no contaminant
...
| `sudokn` | v0.0.10 | 2026-05-08 | Small/medium manufacturer inventory — **attempted and declared insufficient** (§6.4) | Coordinates present on only 225 sites in this release |
...
(47,512 distinct buildings; median modelled maximum depth 1.94 m) and computed its S2
Level-13 cell client-side, verified against the server's own `point_to_s2` conversion. That yields
2,738 flood cells, of which 2,503 fall inside `spatialkg`'s contiguous-US
grid and resolve to 358 counties.
...
|---|---|
| Flood-impacted buildings (UF-OKN) | 47,512 |
| Distinct flood S2 L13 cells | 2,738 |
| Flood cells placed on the spatialkg grid | 2,503 |
| Counties containing ≥1 flood cell | 358 |
| EPA-regulated facilities inside flood cells | 11,085 |
| Flood cells containing ≥1 facility | 1,339 |
| NHDPlus reaches in the federation's network | 507,486 |
| Reaches mapped to a county | 413,434 |
| Source reaches (crossing a facility-bearing flood cell) | 1,101 |
| Routed source→downstream links | 996,529 |
| Distinct downstream reaches | 15,727 |
| Counties in the final typology | 519 |
...
|---|---|---|---|---|
| **A** | Downstream reach in the **same HUC8 subbasin** as the source | Local transport; contaminant residence time short, dilution limited | 1.00 | 57,586 |
| **B** | Same **HUC4 subregion**, different HUC8 | Regional transport; plausible but attenuated | 0.50 | 143,555 |
| **C** | Same **HUC2 region**, different HUC4 | Long-range mainstem transport; strongly attenuated, retain as connectivity only | 0.25 | 370,004 |
| **D** | Different HUC2 region | Cross-region; would be a data artefact — **none observed** | 0.10 | 0 |

Tier C dominates by count (370,004 links), which is exactly what an unbounded transitive closure on
...
Of 11,085 flood-exposed EPA-regulated facilities, 2,270 carry a NAICS
...
together contribute 799 facilities (35.2%), and the three largest
sectors take 45.1% of the total. The sector-level HHI of 908.0 sits in the range
...
> line marks 50%. The HHI and top-3 share are annotated. n = 2,270 NAICS-coded facilities
> of 11,085 total. Provenance: `fiokg` `fio:ofIndustry` (all NAICS hierarchy levels; the
...
The routing produces 996,529 source→downstream reach links from 1,101
source reaches — a mean fan-out of 905 downstream reaches per source, with a long right tail. Those
reaches touch 25,063 S2 cells. Rolled to counties and restricted to genuinely
...
> each of the 1,101 source reaches reaches through `downstreamFlowPathTC`. **(C)**
...
incomplete**: the network is 507,486 reaches across 348 HUC8 subbasins in 10
...
flood-exposed facilities (imported) — over the 519 counties that appear on either axis
gives four groups.

| Typology | Counties | Median co-located facilities | Median imported facilities | % rural (RUCC ≥ 4) | Median population |
|---|---|---|---|---|---|
| **Retained** — contamination stays home | 153 | 13 | 0 | 33% | 107,215 |
| **Imported** — risk generated upstream | 149 | 0 | 402 | 62% | 20,813 |
| **Compound** — heavy on both | 59 | 19 | 353 | 53% | 61,976 |
| Low | 158 | 0 | 0 | 54% | 35,293 |
...
> n = 516 of 519 counties with a RUCC record. Provenance: `ruralkg`
> `settlementtype:hasRUCC` / `population` (2013 vintage) joined on county FIPS to the typology.

Imported counties are 62% rural against 33% for Retained;
their median RUCC is 6 against 3; their median population is
20,813 against 107,215 — roughly a fifth the size. In plain terms: **the
...
The counterfactual the study was built to answer. Ranking the same 519 counties on
**co-located flood-exposed facilities alone** — the co-location baseline — and comparing to the
routing-aware cascade rank:

- **31 of the top 50 counties change** (19 of 50 survive in both).
- Half of the top 100 change (50/100 overlap).
- Spearman ρ = 0.578, Kendall τ = 0.475 — a substantial but far from complete
  correspondence.
- **179 counties (18.5 million residents) score exactly
...
> spatialkg).*** Each point is one of the 519 counties; x = percentile rank of
...
small (59) but contains the highest-stakes places — Madison County IL, Jefferson County KY,
...
route with no transport step to weight or attenuate. `hydrologykg` places **1,006 water
...
- **333 Illinois wells with purpose `WATER`** (water supply) and **23
  Maine wells with use `Domestic`** — 333 + 23 = 356 wells whose
  function is to supply drinking water, sitting inside a modelled flood footprint.
- 419 monitoring wells and 211 engineering wells, plus 13 water-test, 3 irrigation
...
Of the 25,063 downstream S2 cells reached by the routing, only **727
contain a `sawgraph` contaminant-observation or monitoring feature**, spread across
79 counties. Turned around: **151 of the 208
Imported and Compound counties — 8.4 million residents — have no contaminant
monitoring anywhere in their downstream cells** in this federation.

The mismatch is even starker at the source end. `sawgraph` carries 88,007 S2 cells with
contaminant features, but only **13 of them coincide with a UF-OKN flood
...
`sudokn` covering ~42,560 sites. In the release queried here, only **225 sites carry
...
EPA FRS manufacturing (NAICS 31–33, 799 facilities, §5.1) covers the regulated subset of
the same population.
...
orders of magnitude too sparse to characterise agricultural land in a 2,738-cell flood
...
of the top fifty counties change when it is added, and 179 counties move
...
Imported group is 62% rural against 33% for Retained, with a
...
unmonitored.** 151 Imported/Compound counties have no downstream contaminant
...
The complete ranked table — all 519 counties with both risk axes, the typology, the
...
**Findings recap.** Across a national modelled flood footprint of 47,512 buildings
(2,738 S2 cells), 11,085 EPA-regulated facilities sit inside the water's way, with
exposure moderately concentrated in manufacturing and small chemically-active service premises
(HHI 908.0; top-3 sectors 45.1%). Routing those sources through the federation's
NHDPlus downstream closure produces 996,529 links to 15,727 downstream
reaches and a four-way county typology: 153 Retained, 149 Imported,
59 Compound, 158 Low. The imported group is the finding — 179
counties and 18.5 million people carry flood-contamination exposure generated
entirely upstream, they are 62% rural against 33% for the
retained group, and 151 of the Imported/Compound counties have no downstream
contaminant monitoring at all. Dropping the routing step changes 31 of the top 50
counties. Separately, 1,006 water wells — 356 of them drinking-water supply — sit inside
...
3. **The routable network is geographically incomplete.** 507,486 reaches across 348
   HUC8 subbasins in 10 HUC2 regions. Flood cells in the Gulf, South Atlantic, Texas, Great
...
4. **Only 20% of flood-exposed facilities carry a NAICS code** (2,270 of 11,085).
...
6. **The flooded-well count is a two-state floor.** `hydrologykg` carries well inventories only for
   Illinois and Maine; 1,006 wells is what those two states yield, not a national figure.
