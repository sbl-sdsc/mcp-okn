...
![Figure 4](figures/fig4_islet_activity.png)

> ***Figure 4. Disease-associated molecular activity in the islet.*** **(A)** The ten most down- and ten most up-regulated genes in human T2D from the Gene Expression Atlas, annotated with the UBERON tissue; blue = reduced in T2D, red = increased. **(B)** Distribution of log₂(T2D / non-diabetic) open-chromatin gene-activity per islet cell type (PanKbase), boxes ordered by median, dashed line at no change; whiskers exclude outliers. Provenance: gene-expression-atlas-okn `wobd:log2fc` with `direction` and `adj_p_value`; pankgraph `type_2_diabetes__OCR_GeneActivityScore_mean` versus `non_diabetic__OCR_GeneActivityScore_mean`, gene linked by `biolink:located_in`, cell type from the statement object.
