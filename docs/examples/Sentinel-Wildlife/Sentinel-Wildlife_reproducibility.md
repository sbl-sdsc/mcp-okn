...
- **External MCP servers:** PubMed (article search + metadata) · Paperclip (full-text corpus)
...
Analysis performed with the mcp-okn MCP server against the FRINK OKN federated SPARQL endpoint. Static OpenStreetMap basemap tiles were unreachable from the analysis sandbox, so all geography is delivered as an interactive folium/Leaflet OSM map (tiles load client-side in the reader's browser) rather than a static map figure; no bare latitude/longitude scatter is used anywhere in the deliverables. Literature comparison used **two** external connectors. PubMed supplied the search and the citation
metadata for all 14 references. **Paperclip** supplied full text for the two references whose body
text was actually read — Ellis et al. 2021 (PMC8432794: results section plus the published
peer-review exchange) and Fiacre et al. 2023 (PMC10222181: the amplifying-host statement in the
body). No other reference was read beyond its PubMed abstract, and the report says so explicitly in
§8 and §12. Two references central to the report's only CONTRADICTED verdict — Kunkel et al. 2022 in
*Avian Pathology* and Malmberg et al. 2023 in the *Journal of Wildlife Diseases* — are paywalled and
present in neither PMC nor the Paperclip corpus, so those claims rest on structured abstracts. One header field was completed after the fact: the Skills line named both skills without pinning them, and the versions (`okn-bioanalysis v0.1.2`, `okn-report-style v0.1.4`) were added by hand rather than by regenerating the record — they are the versions this repository carried at the build stamped above (`8888193`), which predates the study's window, so they are the versions the run actually followed. No other header field, query or count was touched. Neither connector's own version is recorded, because a session cannot observe it: MCP reports a server's version only in the initialize handshake, which is not exposed to the running agent.
...
### Post-hoc correction of the citation record (2026-07-29)

Two defects in the literature layer were found on review and fixed:

- **Four references were drafted from memory rather than from the NCBI record.** References 9–12
  (Shu et al. 2023, Zhu et al. 2025, Rohonczy et al. 2024 ×2) were written from search-result PMIDs
  without calling `get_article_metadata`, contrary to the rule that every field comes from the NCBI
  record. All four were subsequently re-fetched and checked: titles, journals, years, DOIs and the
  quantitative claims attributed to them (liver ∑PFAS 215.97 ng/g dw; >80 % maternal transfer,
  bioamplification 1.79 / 2.70; BCF ≤ 10 L/kg) are correct as published, and PMC10940426 is a real
  identifier. One title was corrected to the NCBI string verbatim, which carries an upstream typo
  ("Surveillance infrom Inlet Watersheds").
- **Five `— full-text-verified` markers claimed reading that had not happened.** They were applied
  after reading PubMed abstracts only. Full text was then genuinely retrieved through Paperclip for
  the two articles available there, and the markers now appear on exactly those two. The remaining
  three were removed, and §8 now names which claims rest on abstracts and why.
