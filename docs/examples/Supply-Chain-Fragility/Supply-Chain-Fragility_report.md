...
The observed base is **15,571 small and medium manufacturers** across
**36** six-digit NAICS 332 industries, employing **324,853** people as
recorded in `sudokn`; **13,277** of them resolve to one of **1,705**
counties. The wider fabricated-metal sector — every six-digit NAICS 332 code, including the 25
that `sudokn` does not carry — accounts for **27,200** EPA-regulated facilities
across **2,130** counties in `fiokg`. On the software side, `securechainkg` holds
**803,769** packages, **29,574,574** dependency edges and **312,388** CVEs; a
single package, **numpy**, is a direct dependency of **70,032** others —
**8.7%** of every package in the graph — with **requests**
(64,097) and **pandas** (45,991) close behind.
...
**7** industries at least twice as clustered as the sector norm. The extreme case
is **NAICS 332911 (Industrial valve)**, whose 26-industry-adjusted
...
**12** industries in the high tertile. The two rankings are **weakly negatively correlated
(Spearman ρ = -0.16, n = 26)**: the most software-exposed
...
That short list is **8 industries** covering **1,587** placed firms,
**49,718** recorded jobs and **7,069** regulated facilities, led by
...
County, Texas** is a top cluster in **9** of the fourteen candidate industries
examined, with 141 firms — followed by Cuyahoga (OH), Macomb, Wayne, Oakland and Kent
...
All 6 graphs were queried directly; every count in this report traces to a logged SPARQL
...
in-SPARQL crosswalk. It places 13,277 of 15,571 firms (85%); the missing
...
(sector HHI = 0.00644361, i.e. **155.0 effective counties**). An index near
...
manufacturers and hardware vendors, 20 names reaching
21 NAICS/vendor pairs), and an **analyst-assigned process-technology map**
...
**574 of 15,571 firms (3.7%)** record a year of
establishment. Within that 3.7% sample the modal founding decade is the **2000s**
...
> **(B)** Founding-decade histogram for the 574 firms that record
> `hasOrganizationYearOfEstablishment` — 3.7% of the base, annotated on the panel.
...
> narrowest industries, with the whole-sector value (155.0) marked. **(C)** County-
...
> facilities per 100,000 residents (colour and size); grey points: all 2,130 counties
...
dependency of **70,032** distinct packages, **8.7%** of the entire
803,769-package graph, and it is the *direct* in-degree — the transitive figure is larger
still. **requests** (64,097), **pandas** (45,991), **serde**
...
than as zeros**. Maintenance thinness is effectively absent: only **679 of
803,769 packages (0.084%)** carry any `schema:contributor` data, and
...
inventory: the highest-blast copyleft package is **pyqt5** (GPL-3.0) with 2,613
...
29,574,574 edges exceeds the endpoint's query budget — direct in-degree is used throughout and
...
exposure**: **13 of 26** depend directly on one
...
> **(B)** Recorded CVEs on the hardware products of the 20 organisations that
...
NAICS → organisation name → hardware product → CVE. It works, and it is small: **20
normalised organisation names** match between `securechainkg`'s NAICS-classified manufacturer slice
and its hardware-vendor slice, reaching 21 NAICS/vendor pairs and
10,373 hardware products. The load is dominated by information-technology codes —
...
Graph-wide, hardware carries **22,668 distinct CVEs across 58,836 vulnerable
...
are weakly *anti*-correlated across these industries (ρ = -0.16).** Electroplating
...
9 of the fourteen candidate industries examined, holding 22 of 120 observed
industrial-valve firms, 19 of 187 pipe-fitting firms, 21 of 571 electroplaters and 15 of 170 forging
firms — 141 firms and 3,963 recorded jobs in the candidate set alone. Cuyahoga County OH
...
for manufacturing, because 13 of 26 shop-floor
...
The complete ranking of all 36 industries — firm and employment counts, both
...
> index against industry software-fragility score for the 26 industries with ≥60
> placed firms; dotted lines mark the concentrated threshold (1.5) and the software upper-tertile
> (55.8); short-listed industries in orange and labelled. **(B)** The joint-risk ranking
...
industry, and one of them — Harris County — appears in 9.

## 10. Summary of findings & limitations

**Findings recap.** Across 36 fabricated-metal industries, 15,571 small
and medium manufacturers (324,853 recorded jobs), against a sector-wide
27,200 EPA-regulated facilities in 2,130 counties, seven industries are at least twice as geographically
...
(ρ = -0.16), which is the structural reason the economic and cybersecurity
literatures do not converge, and the short list of **8 industries** must therefore be
built from a joint rank rather than a conjunction. Underneath it all, 13
of 26 shop-floor CAD, metrology and PLC packages depend directly on the same
few hubs — numpy above all, with 70,032 direct dependents, 8.7% of
an 803,769-package graph.
...
2. **Firm age is essentially unmeasured.** 3.7% of firms record a founding year.
...
3. **The software layer is not joined to industry by any edge.** The §6.3 mapping is an analyst
   ...
   20 organisation-name matches and reaches only four NAICS 332 firms. Any
   industry-level software conclusion is one substitution away from changing.
4. **Maintainer thinness could not be measured at all.** 0.084% package
...
6. **Blast radius is direct in-degree, not transitive closure.** Transitive reachability over
   29,574,574 edges exceeded the endpoint budget. All blast-radius figures are lower bounds,
