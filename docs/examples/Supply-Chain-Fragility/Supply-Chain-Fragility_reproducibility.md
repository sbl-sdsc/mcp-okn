...
Layer-1 (sudokn), layer-2 (fiokg) and layer-4 (spoke-okn/ruralkg) results are joined end-to-end inside the graphs. Layer 3 (securechainkg) is joined to the others only by NAICS co-classification and by a 20-name organisation bridge; its attachment to specific industries is an explicitly labelled analyst inference. The query log was reset before this final pass because the server-side log is size-capped and had dropped the earliest establishing queries; every query below was re-run verbatim against the same pinned graph versions. One header field was corrected by hand rather than by regenerating the record: the External MCP servers line also listed `WebSearch`, which is a built-in client tool and not an MCP connector, so that field now names only PubMed. Web search DID supply evidence — the supply-chain security and regulatory references, and the CISA advisories behind the software results — and it stays credited where it belongs, in report §12 and the literature-comparison companion. No query, count or other header field was touched.
