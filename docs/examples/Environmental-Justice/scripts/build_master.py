#!/usr/bin/env python3
"""Assemble the Proto-OKN county-level cumulative environmental-justice burden dataset.
...
def p(f):
    """Return the path to data file ``f`` under the input data directory."""
    return str(Path(D) / f)
...
def clean_fips(s):
    """Normalize a value to a zero-padded 5-digit county FIPS code, or None."""
    s = str(s).strip()
...
def load2(fn, val, dtype=float):
    """Load a two-column FIPS/value CSV, cleaning FIPS and coercing ``val`` numeric."""
    ...
    return df.groupby("fips", as_index=False)[val].mean()
...
court = pd.read_csv(p("scales_county.csv"), dtype=str)
court.columns = ["fips", "court_cases"]
...
names["fips"] = names["fips"].map(clean_fips)
names = names.dropna(subset=["fips"]).drop_duplicates("fips")

# SDoH wide
sd = pd.read_csv(p("sdoh_long.csv"), dtype=str)
sd["fips"] = sd["fips"].map(clean_fips)
sd = sd.dropna(subset=["fips"])
sd["value"] = pd.to_numeric(sd["value"], errors="coerce")
sdw = sd.pivot_table(
    index="fips", columns="indicator", values="value", aggfunc="mean"
).reset_index()

# health wide
hl = pd.read_csv(p("health_long.csv"), dtype=str)
hl["fips"] = hl["fips"].map(clean_fips)
hl = hl.dropna(subset=["fips"])
hl["prevalence"] = pd.to_numeric(hl["prevalence"], errors="coerce")
hw = hl.pivot_table(
    index="fips", columns="disease", values="prevalence", aggfunc="mean"
).reset_index()

# PFAS (Maine)
pf = pd.read_csv(p("sawgraph_pfas.csv"), dtype=str)
pf["fips"] = pf["fips"].map(clean_fips)
for c in ["pfas_mean_ngL", "pfas_max_ngL", "pfas_measurements"]:
    pf[c] = pd.to_numeric(pf[c], errors="coerce")

# nikg (2 counties)
nik = pd.read_csv(p("nikg.csv"), dtype=str)
nik["fips"] = nik["fips"].map(clean_fips)
nik["value"] = pd.to_numeric(nik["value"], errors="coerce")
nikw = nik.pivot_table(
    index="fips", columns="metric", values="value", aggfunc="sum"
).reset_index()
...
m = m.merge(
    pf[["fips", "pfas_mean_ngL", "pfas_max_ngL", "pfas_measurements"]],
    on="fips",
    how="left",
)
...
for raw, rate in [
    ("epa_fac", "epa_fac_pc"),
    ("pfas_fac", "pfas_fac_pc"),
    ("enforce_records", "enforce_pc"),
    ("court_cases", "court_pc"),
    ("sud_providers", "sud_pc"),
    ("water_features", "water_pc"),
]:
...
def pct(col):
    """Return the percentile rank (0-1) of column ``col`` across US50 counties."""
...
predictors = {
    ...
    "pfas_fac_pc": "PFAS-type facilities /10k",
    "enforce_pc": "Enforcement records /10k",
    "court_pc": "Federal court cases /10k",
    "rucc": "Rurality (RUCC)",
    "svi": "Social Vulnerability Index",
    "water_pc": "Water-monitoring features /10k",
}
outcomes = {
    "diabetes": "Diabetes",
    "obesity": "Obesity",
...
for pk, pl in predictors.items():
    for ok, ol in outcomes.items():
        if pk not in MM or ok not in MM:
            continue
...
corr.to_csv(Path(OUT) / "correlations.csv", index=False)
...
cm = (
    m[m["in_us50"]][heat_cols]
    .apply(pd.to_numeric, errors="coerce")
    .corr(method="pearson")
)
cm.to_csv(Path(OUT) / "corr_matrix.csv")

# ---------- master + ranking ----------
m_sorted = m.sort_values(["burden_agreement", "burden_index"], ascending=False)
m.to_csv(Path(OUT) / "master_county.csv", index=False)
rank_cols = [
    "fips",
    "name",
    "state",
...
m_sorted[m_sorted["in_us50"]][rank_cols].to_csv(
    Path(OUT) / "burden_ranking.csv", index=False
)
...
META = {
    "epa_fac": (
        "fiokg",
        "facility located in county (FRS, sfWithin)",
        "county",
        "regulatory record",
        "count",
    ),
    "pfas_fac": (
        "fiokg",
        "EPA-PFAS-Facility (industry-flagged) in county",
        "county",
        "regulatory record",
        "count",
    ),
    "enforce_records": (
        "fiokg",
        "enforcement/compliance record for in-county facility",
        "county",
        "regulatory record",
        "count",
    ),
    "water_features": (
        "geoconnex",
        "hydrologic monitoring feature in county (GNIS)",
        "county",
        "monitoring feature",
        "count",
    ),
    "court_cases": (
        "scales",
        "federal court case filed in county (FJC IDB hasIdbCounty)",
        "county",
        "court record",
        "count",
    ),
    "court_pc": (
        "scales",
        "federal court cases per 10k residents",
        "county",
        "court record",
        "per 10k",
    ),
    "rucc": (
        "ruralkg",
        "Rural-Urban Continuum Code (1 urban-9 rural)",
        "county",
        "survey/ranking indicator",
        "code 1-9",
    ),
    "sud_providers": (
        "ruralkg",
        "substance-use treatment providers in county (ZIP->place->county)",
        "county",
        "service listing",
        "count",
    ),
    "poverty": (
        "spoke-okn (SAIPE)",
        "SDoH poverty prevalence in county",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    ...
    "svi": (
        "spoke-okn (CDC SVI)",
        "social vulnerability index in county",
        "county",
        "survey/ranking indicator",
        "0-1",
    ),
    "diabetes": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted diabetes prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "obesity": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted obesity prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "asthma": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted asthma prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "copd": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted COPD prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "stroke": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted stroke prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "cad": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted coronary-artery-disease prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "depression": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted depression prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "hypertension": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted hypertension prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
    "arteriosclerosis": (
        "spoke-okn (CDC PLACES)",
        "age-adjusted arteriosclerosis prevalence (places->county)",
        "county",
        "survey/ranking indicator",
        "%",
    ),
...
for _, r in pf.dropna(subset=["fips"]).iterrows():
    ...
    frows.append(
        [
            r["fips"],
            nm,
            STATE.get(str(r["fips"])[:2], ""),
            "pfas_mean_ngL",
            round(float(r["pfas_mean_ngL"]), 3),
            "ng/L",
            "sawgraph",
            "PFAS water-sample mean, S2->county rollup",
            "county",
            "measured environmental sample",
        ]
    )
    frows.append(
        [
            ...
            STATE.get(str(r["fips"])[:2], ""),
            "pfas_max_ngL",
            round(float(r["pfas_max_ngL"]), 3),
            "ng/L",
            "sawgraph",
            "PFAS water-sample max, S2->county rollup",
            "county",
            "measured environmental sample",
        ]
    )
    frows.append(
        [
            ...
            STATE.get(str(r["fips"])[:2], ""),
            "pfas_measurements",
            int(r["pfas_measurements"]),
            "count",
            "sawgraph",
            "PFAS water measurements, S2->county rollup",
            "county",
            "measured environmental sample",
        ]
    )
...
for _, r in nik.dropna(subset=["fips"]).iterrows():
    ...
    frows.append(
        [
            ...
            "count",
            "nikg",
            "neighborhood incident count in county",
            "county",
            "incident record",
        ]
    )
# scales NIBRS national finding rows
nibrs = pd.read_csv(p("scales_nibrs.csv"), dtype=str)
nibrs["charges"] = pd.to_numeric(nibrs["charges"], errors="coerce")
for _, r in nibrs.iterrows():
    frows.append(
        [
            ...
            "charges",
            "scales",
            "federal charges by NIBRS offense category (national)",
            "national",
            "court record",
        ]
    )
# dreamkg Philadelphia ZIP service finding rows
dk = pd.read_csv(p("dreamkg_services.csv"), dtype=str)
dk["nservices"] = pd.to_numeric(dk["nservices"], errors="coerce")
for _, r in dk.iterrows():
    frows.append(
        [
            str(r["zip"]),
            "Philadelphia ZIP " + str(r["zip"]),
            "PA",
            "dreamkg_services",
            int(r["nservices"]),
            "count",
            "dreamkg",
            "homelessness/social services in ZIP",
            "zip",
            "service listing",
        ]
    )
...
find.to_csv(Path(OUT) / "findings_long.csv", index=False)
...
topN[mat_cols].to_csv(Path(OUT) / "burden_matrix_top40.csv", index=False)


# ---------- summary.json ----------
def cov(col):
    """Return the count of non-null values in column ``col`` (0 if absent)."""
    return int(m[col].notna().sum()) if col in m else 0
...
summ = {
    ...
    "nibrs_categories": len(nibrs),
    "dreamkg_zips": len(dk),
    "burden_agreement_dist": {
        int(k): int(v)
        for k, v in U["burden_agreement"].value_counts().sort_index().items()
    },
    "top_burden": m_sorted[m_sorted["in_us50"]]
    .head(20)[
        [
            "fips",
            "name",
    ...
    .round(3)
    .to_dict("records"),
    "top_correlations": corr.reindex(corr["r"].abs().sort_values(ascending=False).index)
    .head(25)
    .to_dict("records"),
}
with (Path(OUT) / "summary.json").open("w") as f:
...
print("\nTOP correlations:")
print(
    corr.reindex(corr["r"].abs().sort_values(ascending=False).index)
    .head(15)[["predictor", "outcome", "r", "p_value", "n"]]
    .to_string(index=False)
)
