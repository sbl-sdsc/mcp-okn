...
def diverging_heatmap(
    ...
    `rotate_x` controls the x tick labels: None (default) auto-tilts them 45° ONLY when the widest
    column label would collide with its neighbour at the figure's width — so short labels ("D1") stay
    flat while long ones ("D5 Public safety") don't overlap. Pass True/False to force it on/off, or a
    number for a custom angle.
    """
    ...
    if rotate_x is None:
        # Auto: tilt only when the widest label is wider than its per-column slot.
        char_in = (FONT["tick"] + 1) / 72.0 * 0.6  # ~0.6 em per char, in inches
        widest_in = max((len(str(c)) for c in col_labels), default=0) * char_in
        per_col_in = ax.get_figure().get_size_inches()[0] / max(len(col_labels), 1)
        rotate_x = bool(widest_in > 0.7 * per_col_in)
    if rotate_x:
        angle = 45 if rotate_x is True else rotate_x
        plt.setp(
            ax.get_xticklabels(), rotation=angle, ha="right", rotation_mode="anchor"
        )
