...
| **Registry + schema** | Registry built semi-automatically from GitHub metadata; curated repo schemas. | Auto-built from the OKN registry, cached at startup; observed schemas read only from the `okn-void` graph. |
