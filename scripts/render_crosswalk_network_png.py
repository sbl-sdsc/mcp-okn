...
DOM = {c: (l, col) for c, l, col in re.findall(
    r'(\w):\["([^"]+)","(#[0-9A-Fa-f]+)"\]',
    re.search(r"const DOM=\{(.*?)\};", html).group(1))}
i0 = html.find("const R=["); i1 = html.find("\n];", i0)
R = json.loads(html[i0 + len("const R="):i1].rstrip() + "]")
hdr = re.search(r"(\d+) knowledge graphs, (\d+) verified crosswalks \(verified ([^)]*)\)", html)
n_kg, n_xw, date = hdr.group(1), hdr.group(2), hdr.group(3)

eff = lambda w: max(w) if isinstance(w, list) else w


def style(key):
    k = (key or "").lower()
    return (0, (6, 3)) if "ubergraph" in k else ((0, (1, 2)) if "wikidata" in k else "-")
...
nbr = defaultdict(set)
for L in links:
    nbr[L[0]].add(L[1]); nbr[L[1]].add(L[0])
deg = {n: len(nbr[n]) for n in nodeset}

maxlog = math.log10(681045)
lw = lambda w: 0.8 + 6.2 * (math.log10(max(w, 1))) / maxlog
nr = lambda n: min(13, 5 + math.sqrt(deg[n]) * 2)
...
def bez(s, c, t, n=26):
    ...
    return ((1 - ts) ** 2 * s[0] + 2 * (1 - ts) * ts * c[0] + ts ** 2 * t[0],
...
for a, b, dom, w, st, curv, par in sorted(links, key=lambda L: lw(L[3])):
    s = np.array(pos[a]); t = np.array(pos[b]); d = t - s; dl = np.hypot(*d) or 1
    u = np.array([-d[1] / dl, d[0] / dl])
    kw = dict(color=DOM[dom][1], lw=lw(w) * 1.2, alpha=0.65, linestyle=st,
              solid_capstyle="round", dash_capstyle="round", zorder=1)
    if abs(curv) < 1e-9:
        ax.plot([s[0], t[0]], [s[1], t[1]], **kw)
    else:
        c = (s + t) / 2 + u * curv * (0.10 + dl * 0.34) * min(3, par / 2)
        bx, by = bez(s, c, t); ax.plot(bx, by, **kw)
for n in nodeset:
    x, y = pos[n]
    ax.scatter([x], [y], s=(nr(n) * 2.3) ** 2, facecolor="#ffffff",
               edgecolor="#cfccc2", linewidths=1, zorder=3)
    ax.annotate(n, (x, y), xytext=(0, nr(n) * 1.5 + 5), textcoords="offset points",
                ha="center", va="bottom", fontsize=9, color="#1a1a18", alpha=0.72, zorder=4)
dom_h = [Line2D([0], [0], color=c, lw=4, label=l) for (l, c) in DOM.values()]
sty_h = [Line2D([0], [0], color="#5f5e5a", lw=2, linestyle="-", label="direct join"),
         Line2D([0], [0], color="#5f5e5a", lw=2, linestyle=(0, (6, 3)), label="bridged via ubergraph"),
         Line2D([0], [0], color="#5f5e5a", lw=2, linestyle=(0, (1, 2)), dash_capstyle="round", label="bridged via wikidata")]
leg1 = ax.legend(handles=dom_h, loc="lower center", ncol=5, frameon=False, fontsize=9,
                 bbox_to_anchor=(0.5, -0.035), handlelength=1.6, columnspacing=1.4)
ax.add_artist(leg1)
ax.legend(handles=sty_h, loc="lower center", ncol=3, frameon=False, fontsize=8.5,
          bbox_to_anchor=(0.5, -0.08), handlelength=2.4, columnspacing=1.8)
ax.set_title(
    ...
    fontsize=11.5, color="#1a1a18", pad=14)
