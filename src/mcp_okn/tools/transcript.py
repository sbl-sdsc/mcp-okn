    ...
    *,
    ...
    Args:
        ...
        skills: Optional agent skills that shaped the analysis, already normalised by
            ...
            beneath "Generated by" — the model/server versions alone don't say which
            methodology the run followed. Empty = the line is omitted entirely.
    """
...
async def create_chat_transcript(
    ...
    Args:
        ...
        skills: OPTIONAL agent skills that shaped this analysis, each as
            ...
            `metadata.version`. Rendered as a `- **Skills:**` header line, omitted
            when not passed. The server CANNOT see which skills your session
...
async def create_reproducibility_record(
    ...
    Args:
        ...
        skills: OPTIONAL agent skills that shaped this study, each as
            `"<name> v<version>"` (e.g. `["okn-bioanalysis v0.1.0",
            "okn-report-style v0.1.1"]`) — the version is the skill's frontmatter
            `metadata.version`. Rendered as a `- **Skills:**` header line beneath
            "Generated by", omitted when not passed. PASS IT: the model id and server
            version don't tell a replicator which methodology the run followed, and
            the server cannot see which skills your session loaded. List only skills
            you actually followed — naming one you didn't is a phantom source, same
            as an unqueried KG.
