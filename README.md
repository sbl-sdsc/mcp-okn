...
A visual map of the whole network — all 134 crosswalks across 33 graphs, drawn as
...
- **[Cross-KG crosswalk catalog](docs/crosswalks/crosswalks_example.md)** —
  **268 example questions** drawn from the verified crosswalks — two per crosswalk,
  except the spoke-genelab×spoke-okn Entrez join, illustrated twice (G10 expression +
  G17 methylation), for 134 example pairs — across 12 domains (Anatomy & Cell Type, Chemicals, Disease & Phenotype,
  Environmental Toxicology, Function & Pathways, Genes, Geospatial, Industry & Supply
  Chain, Justice & Public Safety, Proteins, Social Determinants & Services, Taxonomy), each with a
  full transcript.
...
| `get_kg_version(shortname=None)` | A KG's release `version`, `last_updated` (ISO-8601 timestamp), and `modified` (month) — read live from the `okn-void` meta-graph's VoID provenance (`pav:version`, `pav:lastUpdatedOn`, `dcterms:modified`). Omit `shortname` for every KG that records provenance (39 of 42), sorted by shortname. Use it to check how current a graph is or cite the exact version behind an analysis. |
